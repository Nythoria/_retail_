...
do
	...
	local function AuctionPriceAppraiser(link)
		...
		local sellPrice, numSeen = AucAdvanced.API.GetAlgorithmValue("Appraiser", link)
...
--		local sellPrice, numSeen = AucAdvanced.Modules.Util.Appraiser.GetPrice(link)
		local imgSeen, image, matchBid, matchBuy, lowBid, lowBuy, aveBuy, aSeen = SimpleAuctionPrivate(link)
