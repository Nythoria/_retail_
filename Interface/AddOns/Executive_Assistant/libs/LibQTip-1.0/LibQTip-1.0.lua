local MAJOR = "LibQTip-1.0.7.3.5.3ExecAssistMod"
...
-- Tables and locals
------------------------------------------------------------------------------
lib.frameMetatable = lib.frameMetatable or {__index = CreateFrame("Frame", nil, nil, "BackdropTemplate")} -- lib.frameMetatable = lib.frameMetatable or {__index = CreateFrame("Frame")}
...
local highlightFrame = CreateFrame("Frame", nil, UIParent, "BackdropTemplate")  -- local highlightFrame = CreateFrame("Frame", nil, UIParent)
...
------------------------------------------------------------------------------
lib.layoutCleaner = lib.layoutCleaner or CreateFrame("Frame", nil, nil, "BackdropTemplate") -- lib.layoutCleaner = lib.layoutCleaner or CreateFrame("Frame")
...
function providerPrototype:AcquireCell()
	...
	if not cell then
		cell = setmetatable(CreateFrame("Frame", nil, UIParent, "BackdropTemplate"), self.cellMetatable) -- 		cell = setmetatable(CreateFrame("Frame", nil, UIParent), self.cellMetatable)
...
function AcquireTooltip()
	...
	if not tooltip then
		tooltip = CreateFrame("Frame", nil, UIParent, "BackdropTemplate") -- 		tooltip = CreateFrame("Frame", nil, UIParent)

		local scrollFrame = CreateFrame("ScrollFrame", nil, tooltip, "BackdropTemplate") -- 		local scrollFrame = CreateFrame("ScrollFrame", nil, tooltip)
		...
		local scrollChild = CreateFrame("Frame", nil, tooltip.scrollFrame, "BackdropTemplate") -- 		local scrollChild = CreateFrame("Frame", nil, tooltip.scrollFrame)
...
function tipPrototype:UpdateScrolling(maxheight)
	...
	if bottomside < 0 or topside > screensize or (maxheight and tipsize > maxheight) then
		...
		if not self.slider then
			local slider = CreateFrame("Slider", nil, self, "BackdropTemplate")  -- 			local slider = CreateFrame("Slider", nil, self)
